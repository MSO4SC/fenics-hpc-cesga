...
//-----------------------------------------------------------------------------
DirichletBC_CG2v_CG1::DirichletBC_CG2v_CG1(Mesh& mesh, SubDomain& sub_domain, uint subsys, Function* val_fun) :
    BoundaryCondition(),
    mesh(mesh),
    sub_domains(NULL),
    sub_domain(0),
...
//-----------------------------------------------------------------------------
DirichletBC_CG2v_CG1::~DirichletBC_CG2v_CG1()
{
...
//-----------------------------------------------------------------------------
void DirichletBC_CG2v_CG1::apply(GenericMatrix& A, GenericVector& b, const Form& form)
{
  apply(A, b, form.dofMaps()[1], form);
}
//-----------------------------------------------------------------------------
void DirichletBC_CG2v_CG1::apply(GenericMatrix& A, GenericVector& b, const DofMap& dof_map,
...
//-----------------------------------------------------------------------------
void DirichletBC_CG2v_CG1::apply(GenericMatrix& A, GenericVector& b, const GenericVector& x,
                   const Form& form)
{
  apply(A, b, form.dofMaps()[1], form);
}
                   ...
                   const Form& form)
{
  ...
  message("Applying DirichletBC_CG2v_CG1 boundary conditions to linear system.");
  ...
  if (boundary->numCells())
  {
	...
	// 1 cg1 + gdim cg1v
	 ...
	 value_fun->eval(values, vertex.x());
        ...
        applyDirichletBC_CG2v_CG1((Matrix&) A, *As, (Vector&) b, mesh, node, nodes, values[0]);
        count++;
        nodes.clear();
      }
...
//-----------------------------------------------------------------------------
void DirichletBC_CG2v_CG1::init(SubDomain& sub_domain)
{
...
//-----------------------------------------------------------------------------
void DirichletBC_CG2v_CG1::applyDirichletBC_CG2v_CG1(Matrix& A, Matrix& As, Vector& b, Mesh& mesh,
