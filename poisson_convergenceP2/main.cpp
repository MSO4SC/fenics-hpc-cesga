...
#include "ufc2/L2error2D.h"
#include "ufc2/L2error3D.h"
...
{
  class ExactSoln : public Function
  {
  public:
    
    ExactSoln(Mesh& mesh) : Function(mesh) {
    ...
    {
     if (geom_dim == 2)
     	value[0] = sin(DOLFIN_PI*x[0])*sin(DOLFIN_PI*x[1]);

     if (geom_dim == 3)
        value[0] = sin(DOLFIN_PI*x[0])*sin(DOLFIN_PI*x[1])*sin(DOLFIN_PI*x[2]);
    }
...
//  DirichletBC bc(u0, mesh, boundary);
  DirichletBC_CG2v_CG2 dbc(mesh, boundary, 0, &u0);
 ...
 // message("Applying BCs took : %f" ,time()- time_v);
  ...
  KrylovSolver solver(bicgstab, none);
  time_v = time();
  solver.solve(A,x,b);
...
//  message("after apply");
  ...
  u.eval(value, coords);
  std::cout << "value is : " << value[0] << std::endl;
  message("after eval");
  real value_global; 
  ...
  if (mesh.topology().dim() == 2)
  	message("Error in (%g, %g) : %g= %g - %g", coords[0], coords[1],  fabs(value_global - sin(DOLFIN_PI*coords[0])*sin(DOLFIN_PI*coords[1]) ), value_global, sin(DOLFIN_PI*coords[0])*sin(DOLFIN_PI*coords[1]));
  else
